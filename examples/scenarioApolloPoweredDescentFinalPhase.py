...
initial_fuel_mass = 600
full_mass = 10334.0 - 7500 + initial_fuel_mass
scObject.hub.mHub = full_mass
dry_mass = full_mass - initial_fuel_mass
...
# oe.e = 0.9  # Add slight eccentricity to introduce vertical component
# oe.i = 0.0 * macros.D2R
# oe.Omega = 0.0
# oe.omega = 0.0
# oe.f = 270 * macros.D2R # Start at quarter past apoapsis (descending)
# rN, vN = orbitalMotion.elem2rv(mu, oe)
rN = np.array([0.0, 0.0, moon_radius + 3000])  # 3km altitude, directly above surface
vN = np.array([150.0, -30.0, 0.0])  # Some horizontal velocity, realistic descent rate
...
initialMass = 10334.0 - 7500 + initial_fuel_mass
...
for step in range(int(simulationTime / simulationTimeStep)):
    ...
    h_burn = burn_margin * speed**2 / (2 * max_a) if max_a > 0 else 0
    ...
    if altitude < h_burn:
        ...
        acc_cmd = 0.1 * max_thrust / current_mass
        retro_dir = -v / np.linalg.norm(v)
        desired_acceleration = acc_cmd * retro_dir

    else:
        # Throttle based on descent speed
        raw_throttle = min(1.0, abs(vertical_velocity) / 100) if vertical_velocity < 0 else 0.0

        if raw_throttle == 0:
            throttle = 0.0
        elif raw_throttle > 0.1 and raw_throttle < 0.6: 
            throttle = raw_throttle 
        else:  # We're in panic territory
            throttle = 1.0  # Full emergency burn
    ...
    if fuel_mass > 0:
        ...
        print(f"pushing with {required_thrust}")
    else:
        thrustForce.extForce_N = [[0.], [0.], [0.]]
